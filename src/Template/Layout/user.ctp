...
$(document).ready(function() {
	...
	$(document).ready(function () {
		$('div.annual_reports').block({ message: 'Please wait' }); 
		$('div.circulars').block({ message: 'Please wait' }); 
		$.ajax({
		  url: "/users/circulars",
		  cache: false,
		...
		$.ajax({
		  url: "/users/getAnnualReports",
		  cache: false,
		  success: function(html){
			$('.annual_reports').html(html);
		  }
		});				
		
		$('body').on('click', '.ar_pagelinks a', function(event){
			...
			$.ajax({
			  ...
			  success: function(html){
				$('.annual_reports').html(html);
				$('div.annual_reports').unblock(); 
			  }
			});			
		});
		
		$('body').on('click', '.circular a', function(event){
			event.preventDefault();
			$('div.circulars').block({ message: 'Please wait' }); 
			var href;
			href = $(this).attr('href');
